...
from .qtdesign import DosAppWindow
from .vaspdostools import utils as vdt
...
class DosAppController(AppController):
    ...
    def remove_dataset(self):
        ...
        if self.view.ax.lines:
            self.view.ax.lines.remove(
                self.view.ax.lines[self.view.datasets_list.currentRow()]
            )
            self.populate_items()
        self.toggle_plot()
    ...
    def export_dataset(self):
        ...
        if len(self.view.ax.lines) > 0:
            ...
            path_to_save = os.path.dirname(self.active_path)
            np.savetxt(
                f"{path_to_save}/{to_save._label}.csv",
                to_save._xy,
                fmt="%.7f",
                delimiter=",",
            )
            ...
            if len(self.view.ax.lines) > 0:
                ...
                ymax = (
                    max(
                        [
                            (
                                line._y[np.where((line._x >= low) & (line._x <= high))]
                            ).max()
                            for line in self.view.ax.lines
                        ]
                    )
                ...
                self.view.ax.vlines(
                    0, 0, ymax, colors="gray", linestyles="dashed", linewidth=4
                )
