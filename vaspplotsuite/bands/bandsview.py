...
from .qtdesign import BandsAppWindow
from .vaspbandtools.bandstructure import BandStructure
...
class BandsAppController(AppController):
    ...
    def load_data(self):
        ...
        try:
            ...
            if "x2-y2" in self.loaded_data.levels:
                self.loaded_data.levels[
                    np.where(self.loaded_data.levels == "x2-y2")
                ] = "dx2"
    ...
    def plot_added_data(self, atoms, states, spin, name, color):
        ...
        self.view.ax.scatter(
            new_xaxis, flattened_bands, s=markers, color=color, label=sel_dataset.name
        )
    ...
    def plot_basic_bands(self):
        ...
        if self.view.ax.lines:
            ...
            low, high = (
                self.loaded_data.bands.min() - 1,
                self.loaded_data.bands.max() + 1,
            )
        for i in range(self.loaded_data.nbands):
            self.view.ax.plot(
                self.loaded_data.xaxis,
                self.loaded_data.bands[0, :, i],
                color="black",
                label="_nolegend_",
            )
        self.view.ax.hlines(0, 0, self.loaded_data.xaxis[-1], color="black", ls="--")
        vlines = self.loaded_data.xaxis[
            np.where(self.loaded_data.xaxis[1:] - self.loaded_data.xaxis[:-1] == 0)
        ]
        ...
        self.view.ax.set_xlim(
            [self.loaded_data.xaxis.min(), self.loaded_data.xaxis.max()]
        )
