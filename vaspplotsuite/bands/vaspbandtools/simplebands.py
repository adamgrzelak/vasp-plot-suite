...
class SimpleBands:
    ...
    def _get_kpoints(self):
        ...
        kpoints = np.array(
            [
                kpoint.text.split()
                for kpoint in self.xml.findall("kpoints/varray[@name='kpointlist']/v")
            ]
        ).astype(np.float32)
        return kpoints
    ...
    def _get_data(self):
        ...
        Read eigenvals
        """
        band_sets = self.xml.findall(
            "calculation/projected/eigenvalues/array/set/set/set/r"
        )
        ...
        if data.shape[0] == self.nkpts:
            ...
            raise TypeError(
                "Something went wrong. Is the data set spin-polarized or not?"
            )
